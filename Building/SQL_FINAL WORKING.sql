DROP SCHEMA IF EXISTS petadoption3;
CREATE DATABASE petadoption3;
use petadoption3;
SHOW TABLES;
...
('Rocky', 'Boxer', 'Male', 2, 2, 21),
('Luna', 'Yorkshire Terrier', 'Female', 3, 2, 19),
('Lucy', 'Chihuahua', 'Female', 1, 11, 16),
('Charlie', 'German Shepherd', 'Male', 4, 11, 17),
('Molly', 'Beagle', 'Female', 2, 27, 14),
('Coco', 'French Bulldog', 'Female', 1, 27, 20),
('Buddy', 'Golden Retriever', 'Male', 5, 17, 11),
('Max', 'Labrador Retriever', 'Male', 3, 17, 9),
('Zeus', 'Siberian Husky', 'Male', 1, 21, 8);
...
-- Write a query to retrieve the names of all the shelters in the database.
SELECT ShelterName FROM Shelter;
...
-- Write a query to retrieve the names of all the donors who have donated more than $1000. 
SELECT FirstName, LastName
FROM Donor
WHERE Amount > 500

-- Write a query to retrieve the names and phone numbers of all the fosters 
-- who are currently fostering a pet and have been fostering for more than 3 months.
SELECT FosterName, FosterPhone
FROM Foster
WHERE FosterStatus = 'Active'
AND DATEDIFF(CURRENT_DATE, StartDate) >= 90
AND (EndDate IS NULL OR EndDate > CURRENT_DATE);


-- Write a query to find the top 5 number of the pets that have been adopted the most number of times.
SELECT PetName
FROM Pet
JOIN AdoptionApplication ON Pet.PetID = AdoptionApplication.PetID
GROUP BY Pet.PetID
ORDER BY COUNT(*) DESC
LIMIT 5;
